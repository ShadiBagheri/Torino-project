...
function EditInfo({ isInfoOpen, setIsInfoOpen ,emailUpdateHandler }) {
    ...
    const validate = () => {
        let error = {};
        if (!email.length) error.email = "لطفا ایمیل را به درستی وارد کنید!";
        return error;
    };
            ...
            <div className="flex flex-col md:flex-row items-center justify-between w-full mx-auto mt-4 md:mt-8 lg:mt-4 lg:pr-5 lg:pl-3 md:gap-10">
                ...
                <form onSubmit={editInfoHandler} className="md:hidden flex items-center justify-between w-full mt-5 md:mt-0 gap-2">
                    <div className="flex w-full">
                        <input
                            className="w-full h-[40px] px-3 text-sm font-normal text-gray-600 border-2 border-gray-500 rounded-[5px] outline-none"
                            type="email"
